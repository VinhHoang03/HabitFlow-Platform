            ...
            child: Column(
              ...
              children: [
                ...
                _AccountActionTile(
                  ...
                  label: 'Hồ sơ',
                  onTap: () =>
                      Navigator.pop(sheetContext, _AccountAction.profile),
                ),
                _AccountActionTile(
                ...
                _AccountActionTile(
                  ...
                  label: 'Đăng xuất',
                  onTap: () =>
                      Navigator.pop(sheetContext, _AccountAction.logout),
                ),
              ],
            ),
          ),
  ...
  Widget _buildHabitList(HabitProvider habitProvider) {
    ...
    if (userId != null) {
      return HabitListTiimoView(
        ...
        onEditHabit: (habit) =>
            _showHabitFormDialog(userId: habit.userId, habit: habit),
        onOpenHabit: (habit) {
    ...
    if (habitProvider.errorMessage != null && habitProvider.habits.isEmpty) {
      return Center(
        child: Padding(
          ...
          child: Text(habitProvider.errorMessage!, textAlign: TextAlign.center),
        ),
      );
    }
      ...
      children: [
        ...
        Expanded(
          child: filteredHabits.isEmpty
              ? const Center(child: Text('Không có thói quen phù hợp bộ lọc.'))
              : _HabitListView(
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: Colors.white,
        border: Border(
          top: BorderSide(color: const Color(0xFFC8C5CF).withOpacity(0.2)),
        ),
        boxShadow: [
          BoxShadow(
        ...
        child: Column(
          ...
          children: [
            Icon(
              icon,
              color: selected
                  ? const Color(0xFF685F72)
                  : const Color(0xFF47464E),
              size: 24,
            ),
...
String _cleanTemplateText(String value) {
  const replacements = {
    ...
    'Äá»c sĂ¡ch 20-30 phĂºt má»—i ngĂ y.': 'Đọc sách 20-30 phút mỗi ngày.',
    'Há»c táº­p': 'Học tập',
...
class _HabitFormDialog extends StatefulWidget {
  ...
  const _HabitFormDialog({required this.userId, this.habit, this.initialHabit});
...
class _HabitSelectField extends StatelessWidget {
  ...
  const _HabitSelectField({required this.label, required this.child});
